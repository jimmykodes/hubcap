...
func (sf SearchFilters) whereClause(searchFields, filterFields fields) *whereClause {
	wc := &whereClause{}
...
func (wc *whereClause) and(f, operand string) {
	wc.writeClause(format(f, operand), "and")
}
func (wc *whereClause) or(f, operand string) {
	wc.writeClause(format(f, operand), "or")
}

func (wc *whereClause) addArg(arg interface{}) {
	wc.args = append(wc.args, arg)
}
func format(f, operand string) string {
	return fmt.Sprintf("`%s` %s ?", f, operand)
}
